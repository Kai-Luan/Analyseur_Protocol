...
public class DNS implements Couche7 {
	...
	private int calcule_RR_Simple(Donnees trame, int indice, String[] champs) {
		...
		indice = calcule_name(trame, 12, sb);
		champs[0] = sb.toString();
		// Calcule le type
		champs[1] = "Type: "+calculeType(trame, indice);
		indice+= 2;
		// Calcule la classe
		int val = trame.parseInt(indice, indice+1);
		if (val==1) champs[2] = "Class: IN (0x0001)";
		else champs[2] = trame.parseHexa(indice, indice+1);
		indice+=2;
		return indice;
	}
	...
	private int calcule_name(Donnees trame, int indice, StringBuilder sb) {
		...
		while (length!=0) {
			...
			// Si c'est un nom non-compressé
				...
				for  (indice+=1; trame.parseInt(indice)!=0; indice++) {
	...
	private String calculeType(Donnees trame, int indice) {
		int type = trame.parseInt(indice, indice+1);
		switch(type) {
		case 1:
			return "A (0x0001)";
		case 2:
		...
		case 28:
			return "AAAA  (0x001C)";
		default:
			return trame.parseHexa(indice, indice+1);
		}
	}
